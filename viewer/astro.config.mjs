...
for (const p of data.parts) if (p.unverified) noindex.add(`/parts/${p.id}/`);
...
export default defineConfig({
  ...
  vite: {
    ...
    // Indexable unless SITE_INDEXABLE=false (see lib/seo.ts).
    define: { __SITE_INDEXABLE__: JSON.stringify(process.env.SITE_INDEXABLE ?? 'true') },
  },
});
