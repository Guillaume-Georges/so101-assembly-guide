...
import { cablesAt, installedAt, jointOf, visibility, type Placement, type StepLite } from './state';
...
describe('island state', () => {
  ...
  it('shows a fastener only where its host part is installed by that step', () => {
    const f = P({ kind: 'fastener', id: 'motor-tab-screw', host: ['base-so101'] });
    expect(installedAt(f, steps)).toBe(2);
    expect(
      installedAt(
        P({ kind: 'fastener', id: 'motor-tab-screw', host: ['moving-jaw-so101'] }),
        steps,
      ),
    ).toBe(-1);
  });
